...
Calendar.prototype = {
	...
	_init: function(config) {
        ...
        **/
		this.callback = {
		    buildDomDay: config.callback && config.callback.buildDomDay || null
		};
		...
		while(arr.length) {
			for(i = 0, len = arr.length; i < len; i++) {
				if(arr.length) {					
					...
					if (sValue && typeof this.callback.buildDomDay === "function") {
					    this.callback.buildDomDay.apply(this, [oA]);
					}
				}
			}
		}
