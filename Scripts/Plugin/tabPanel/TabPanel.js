...
TabPanel.prototype = {
    ...
    addTab: function (tabitem) {
        ...
        if ($('#' + tabitem.id).length > 0) {
            ...
            var activedTabIndex = this.tabpanel_mover.children().index(this.tabpanel_mover.find('.active_wy')[0]);
            ...
            }(this.tabs.length - 1));//(this.tabs[this.tabs.length - 1].id));
     ...
     * @return {Number} index of tab.   wei.yao ===>根据ID找出索引
     */
    ...
    show: function (position, notExecuteMoveSee) {
        ...
        if (this.scrollFinish) {
            ...
            if (this.tabs[position].tab.hasClass('active_wy')) {
                ...
                this.tabpanel_mover.find('.active_wy').removeClass('active_wy');
                this.tabs[position].tab.addClass('active_wy');
                if (!notExecuteMoveSee) {
                    this.moveToSee(position);
                }
            }
   ...
   */
    getActiveIndex: function () {
        return this.tabpanel_mover.children().index(this.tabpanel_mover.find('.active_wy')[0]);
    },
    ...
    getActiveTab: function () {
        var activeTabIndex = this.tabpanel_mover.children().index(this.tabpanel_mover.find('.active_wy')[0]);
