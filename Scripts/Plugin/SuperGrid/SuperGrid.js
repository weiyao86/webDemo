...
define("SuperGrid", ["ajax",
    ...
    function (ajax, consts, Mustache, Settings) {
        ...
        // default callback
        var defaultCallbacks = (function () {
            return {
                rowClicked: null,
                deleteBefore: null,
                deleteAfter: null,
                onRenderAfter: null,
                openEditDialogBefore: null,
            };
        })();

        // defined 'SuperGrid' constructor
        var SuperGrid = function (options, callbacks) {
            ...
            // callback
            this.cabs = $.extend({}, defaultCallbacks, callbacks || {});
            ...
            this.systemTableName = this.opts.validation.systemTableName;
            this.validationRuleUrl = Settings.context.validationRuleUrl;

            // search
            this.$search = $("#" + this.opts.search.id);

            // grid
            this.keys = this.opts.grid.keys;
            this.$grid = $("#" + this.opts.grid.id);
            this.$toolbar = $("#" + this.opts.grid.toolBar);
            ...
            this.$topPage = $("#" + this.opts.paging.topPageId);
            this.$prevPage = $("#" + this.opts.paging.prevPageId);
            this.$nextPage = $("#" + this.opts.paging.nextPageId);
            this.$bottomPage = $("#" + this.opts.paging.bottomPageId);
            this.$gotoPage = $("#" + this.opts.paging.gotoPageId);
            this.$currentPage = $("#" + this.opts.paging.currentPageId);
            this.$recordCount = $("#" + this.opts.paging.recordCountId);
            this.$pageCount = $("#" + this.opts.paging.pageCountId);

            // edit
            this.$edit = $("#" + this.opts.edit.id);
            this.dialogWidth = this.opts.edit.dialogWidth || this.$edit.width();
            this.dialogHeight = this.opts.edit.dialogHeight || this.$edit.height();

            // callbacks
            this.rowClicked = this.cabs.rowClicked;
            this.deleteBefore = this.cabs.deleteBefore;
            this.deleteAfter = this.cabs.deleteAfter;
            this.onRenderAfter = this.cabs.onRenderAfter;
            this.openEditDialogBefore = this.cabs.openEditDialogBefore;

            // initialization
            this.init();
        };

        // extend 'SuperGrid' constructor prototype
        SuperGrid.prototype = (function () {
            return {
                /**************
                 Common block
                ***************/
                ...
                init: function () {
                    ...
                    self.bindGridHeaderEvent();

                    // setting grid width
                    if (self.opts.grid.isAutoWidth) self.settingGridWidth();

                    // fixed grid column
                    self.fixedGrid();

                    // window resize after reset fixed grid 
                    $(window).resize = $.proxy(self.fixedGrid, self);

                    // bind search permission
                    //Permission.bind([self.$search, self.$toolbar, self.$edit]);
                    ...
                    // bind export event
                    self.bindExportEvent();

                    // Append export form
                    self.appendExportForm();

                    // register edit text box constraint 
                    self.registerConstraint();

                    // build grid template
                    self.gridTemplate = $("#" + this.opts.grid.templateId).html();

                    // first load to render grid
                    if (self.opts.grid.firstRender) {
                        self.params = self.getCondition();
                        self.invoke(self.params);
                    }
                },

                // setting column fixed
                fixedGrid: function () {
                    var self = this;

                    if (!($.browser.msie && parseInt($.browser.version) < 9)) {
                        self.fixedGridColumn();
                    }
                },
                ...
                getCondition: function () {
                    ...
                    $.each(self.opts.grid.filters, function (index, item) {
                ...
                **************/
                ...
                bindSearchEvent: function () {
                    var self = this;
                    var selector = "input[data-area='search'][type='text']";

                    //keyup search
                    self.$search.find(selector).keyup(function (e) {
                ...
                **************/
                ...
                render: function (data) {
                    var self = this;
                    var $template = $("#" + self.opts.grid.templateId);
                ...
                renderAfter: function ($template) {
                    ...
                    if (self.opts.grid.colResizable) self.$grid.colResizable();
                ...
                bindGridHeaderEvent: function () {
                    ...
                    $("#" + self.opts.grid.addButtonId).click(function (e) {
                ...
                bindGridBodyEvent: function () {
                    ...
                    with (self) {
                        ...
                        $grid.find("a[id*='" + self.opts.grid.deleteButtonId + "']").click(function (e) {
                            self.deleteSettings(this, e);
                            e.stopPropagation();
                        });
                        $grid.find("a[id*='" + self.opts.grid.editButtonId + "']").click(function (e) {
                ...
                *************/
                ...
                bindPagingEvent: function () {
                    ...
                    self.$bottomPage.click(function () {
                        if (self.pageCount > 1 && self.currentIndex < self.pageCount) {
                            self.bottomPage();
                            self.invoke(params);
                        }
                    ...
                    self.$gotoPage.click(function () {
                        self.gotoPage();
                        self.invoke(params);
                    });

                    self.$currentPage.keyup(function (e) {
                        if (e.keyCode === 13) {
                            self.gotoPage();
                            self.invoke(params);
                        }
                ...
                gotoPage: function () {
                    ...
                    var gotoPageIndex = self.$currentPage.val();
                ...
                settingPageButtonStatus: function () {
                    ...
                    self.$topPage.removeClass("first_icon_hover");
                    self.$prevPage.removeClass("pre_icon_hover");
                    self.$nextPage.removeClass("next_icon_hover");
                    self.$bottomPage.removeClass("last_icon_hover");
                    
                    if (self.currentIndex > 1 && self.currentIndex < self.pageCount) {
                        self.$topPage.addClass("first_icon_hover");
                        self.$prevPage.addClass("pre_icon_hover");
                        self.$nextPage.addClass("next_icon_hover");
                        self.$bottomPage.addClass("last_icon_hover");
                    }
                ...
                *************/
                bindEditEvent: function () {
                    ...
                    $("#" + self.opts.edit.saveId).click(function () {
                        self.saveRecord();
                    });

                    //registger cancel button click event
                    $("#" + self.opts.edit.cancelId).click(function () {
                ...
                *************/
                ...
                bindExportEvent: function () {
                    var self = this;
                    $("#" + this.opts.exports.exportButtonId).click(function () {
                ...
                appendExportForm: function () {
                    ...
                    var $form = $("<form>", {
                        id: "export-form",
                        action: self.opts.exports.exportUrl,
                        method: "post"
                    });
                    ...
                    $form.append($hidden);
                    $("#" + this.opts.exports.formWrapId).append($form);
                },
